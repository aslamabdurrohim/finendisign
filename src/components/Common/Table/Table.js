...
const settingsColumns = (onEditDesignClick, onChangeBackground, idBackground) => [
    {
        ...
        columns: [
            ...
            },
            {
                Header: "Title",
                id: "title",
                Cell: row => <div className={styles.settings_columns}>{row.original.title}</div>,
                style: { "white-space": "unset", "line-height": "1.5" }
            },
            {
                Header: "Category",
                id: "category",
                Cell: row => <div className={styles.settings_columns}>{row.original.category}</div>,
                width: 120
            },
            {
                Header: "",
                ...
                Cell: row => (
                    <div className={styles.settings_columns}>
                        <div className={styles.settings_action_column}>
                            ...
                            <div
                                onClick={e => onChangeBackground(row.original.id)}
                                role="presentation"
                            >
                                <FontAwesomeIcon
                                    icon="images"
                                    size="1x"
                                    style={{
                                        color: row.original.id === idBackground ? "#4caf50" : "#333"
                                    }}
                                />
                            </div>
                        </div>
...
const Table = ({ data, onChangeBackground, onEditDesignClick, idBackground, type }) => (
    <ReactTable
        data={data}
        noDataText="No data found"
        columns={
            type === "settings"
                ? settingsColumns(onEditDesignClick, onChangeBackground, idBackground)
                : null
        }
        defaultPageSize={10}
        className="-highlight"
